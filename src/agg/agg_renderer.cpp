...
template <typename T0, typename T1>
void agg_renderer<T0,T1>::render_marker(symbolizer_base const& sym,
                                    ...
                                    composite_mode_e comp_op)
{
    ...
    if (marker.is_vector())
    {
        ...
        bool result = push_explicit_style( (*marker.get_vector_data())->attributes(), attributes, sym, feature, common_.vars_);
        svg_renderer_agg<svg_path_adapter,
                         svg_attribute_type,
                         renderer_type,
                         pixfmt_comp_type> svg_renderer(svg_path,
                                                        result ? attributes : (*marker.get_vector_data())->attributes());
